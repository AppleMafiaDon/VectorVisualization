...
Point drawVector(baseVector vector, double length, double lineWidth)
{
	...
	//Tip
	glBegin(GL_POLYGON);
		glVertex2d(endPoint.x + 0.25*cos(vector.angle-15), endPoint.y + 0.25*sin(vector.angle-15));
		glVertex2d(endPoint.x, endPoint.y);
		glVertex2d(endPoint.x + 0.25*cos(vector.angle+15), endPoint.y + 0.25*sin(vector.angle+15));
	glEnd();
...
void drawVector(Point origin, Point endPoint, double angle, double lineWidth)
{
	...
	glBegin(GL_POLYGON);
		glVertex2d(endPoint.x - 0.25*cos(degToRad(angle+20)), endPoint.y - 0.25*sin(degToRad(angle+20)));
		glVertex2d(endPoint.x, endPoint.y);
		glVertex2d(endPoint.x - 0.25*cos(degToRad(angle-20)), endPoint.y - 0.25*sin(degToRad(angle-20)));
	glEnd();
	glDisable(GL_LINE_WIDTH);
...
void myDisplay(void)
{
	...
	double angle = 0.0, radius = 3.5;
	
	for(int i = 0; i < 6; i++)
	{
		vectors[i].origin.draw();
		drawVector(vectors[i], radius, 2);
		glColor3f(0.25,0.75,0.125);
		endPoints[i] = drawVector(vectors[i], radius * scalars[0][i], 1.2);
		//if(i > 0)
		//{
		//	//Quadrant I
		...
		//		angle = 90 + getDegFromSlope(endPoints[i], endPoints[i-1]);
		//	//Quadrant III
		//	else if((endPoints[i].y - endPoints[i-1].y) < 0 && (endPoints[i].x - endPoints[i-1].x) < 0)
...
void main(int argc, char** argv)
{
	...
	glutCreateWindow("Problem 1 - Stage 2"); // open the screen window
